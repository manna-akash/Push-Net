...
class PushController:
    def __init__(self):
        ...
        self.bs = args.batch_size
        ''' goal specification '''
        self.w = -70 # orientation in degree (positive in counter clockwise direction)
        ...
        self.pose_params =(self.w, self.x, self.y)
        ## instantiate Push-Net predictor
        self.pred = Predictor()
        Ic = cv2.imread('/home/tomo/blister/PushNet/target_pose.png')[:,:,0]
        #TODO :Remove it
        Ic = generate_goal_img(Ic, self.w, self.x, self.y)
        cv2.imwrite(f"./initial_pose/{self.pose_params[0]}d_{self.pose_params[1]}x_{self.pose_params[2]}y.png", Ic)
    ...
    def get_best_push(self, Ic):
        ...
        img_in_curr_ = cv2.imread("./target_pose.png").astype(np.uint8)[:,:,0]
        #TODO: Remove roration
        #img_in_curr_ = cv2.rotate(img_in_curr_, cv2.ROTATE_180)
        _, img_in_curr_ = cv2.threshold(img_in_curr_.copy(), 30, 255, cv2.THRESH_BINARY)

        ''' generate goal image '''
        img_in_next =  generate_goal_img(img_in_curr_.copy(), 0, 0, 0)#cv2.imread("./results/target_poses/target_-11.25d_10x_-10y.png")#

        ''' visualize goal image '''
        if NEXT_VIS:
            cv2.imshow('Target Pose', img_in_next)
            cv2.imwrite(f"./results/target_poses/target_{self.w}d_{self.x}x_{self.y}y.png",img_in_next)
            cv2.waitKey(0)
        ...
        if BEST_VIS:
            self.draw_action(img_in_curr.copy(), best_start, best_end, single=True, sub = f"best_{self.pose_params[0]}d_{self.pose_params[1]}x_{self.pose_params[2]}y")

        if SAMPLE_ACTIONS:
            _sub = f"{self.pose_params[0]}d_{self.pose_params[1]}x_{self.pose_params[2]}y"
            for num_act in range(NUM_ACTION_EXECUTE):
                sub= _sub +"_"+str(num_act)
